...
def generar_diseno(data_input, color_version="AMARILLO"):
    ...
    if formato == "FLYER":
        ...
        if "EFERTON" in tipo or es_aniv:
            if formato == "PPL":
                pi.thumbnail((647, 670))
                px_img = 277 if es_aniv else 156
                img.paste(pi, (px_img, 243), pi)

                f_marca_ppl = ImageFont.truetype(f"{path_fonts}/Poppins-Medium.ttf", 30)
                mx = 990 if es_aniv else 90
                m_anchor = "rs" if es_aniv else "ls"
                draw.text((mx, 930), row['Marca'], font=f_marca_ppl, fill=(255,255,255), anchor=m_anchor)
                ...
                if hay_precio:
                    draw_efe_preciador(draw, prec_x, 1650, "S/", precio_val, ImageFont.truetype(f"{path_fonts}/Poppins-ExtraBold.ttf", 64), ImageFont.truetype(f"{path_fonts}/Poppins-ExtraBold.ttf", 110), scale=1.1, padding_h=30)
                ...
                px_img = 180 if es_aniv else 440
                cx = 820 if es_aniv else 260

                pi.thumbnail((460, 460)); img.paste(pi, (px_img, 25), pi)
