...
def generar_diseno(data_input, color_version="AMARILLO"):
    ...
    if formato == "FLYER":
        ...
        draw.text((x_fecha, 260), f_txt, font=f_f, fill=color_blanco, anchor="lm")
        ...
        for i, (idx, p) in enumerate(data_input.iterrows()):
            ...
            xp = 64 + (i % 2) * (box_w + 40); yp = y_base + (i // 2) * (box_h + gap_y)
            # AJUSTE: Se quita el borde azul de la caja blanca de cada producto (sin outline)
            draw.rounded_rectangle([xp, yp, xp+box_w, yp+box_h], radius=15, fill=(255,255,255))
            try:
            ...
            ny = y_marca_prod + 25 
            # AJUSTE: nombre auto-ajustable (máx 2 líneas, reduce fuente si es muy largo). Centrado.
            lineas_nombre, f_nom, alto_l = ajustar_nombre(draw, p['Nombre del producto'], ruta_bold, size_max=18, size_min=12, ancho_max=200, max_lineas=2)
            for ln in lineas_nombre:
                draw.text((cl, ny), ln, font=f_nom, fill=azul_oscuro, anchor="mm"); ny += alto_l
        ...
        if formato == "DISPLAY":
            ...
            lineas_nombre, f_nom, alto_l = ajustar_nombre(draw, row['Nombre del producto'], ruta_bold, size_max=32, size_min=22, ancho_max=460, max_lineas=3)
            for l in lineas_nombre: draw.text((cx_textos, ny), l, font=f_nom, fill=txt_c, anchor="lt"); ny += alto_l
