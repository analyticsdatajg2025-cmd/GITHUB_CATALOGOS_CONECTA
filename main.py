...
def generar_diseno(data_input, color_version="AMARILLO"):
    ...
    try:
        ...
        if formato == "STORY":
            ...
            f_pv = ImageFont.truetype(f"{path_fonts}/HurmeGeometricSans1 Bold.otf", 73); f_s_fly = ImageFont.truetype(f"{path_fonts}/HurmeGeometricSans1.otf", 13); f_l = ImageFont.truetype(f"{path_fonts}/HurmeGeometricSans1.otf", 16); f_p = ImageFont.truetype(f"{path_fonts}/HurmeGeometricSans1 Bold.otf", 18); f_ps = ImageFont.truetype(f"{path_fonts}/HurmeGeometricSans1 Bold.otf", 30) 
        else:
...
filas_para_google = [] # <--- LISTA PARA ACUMULAR DATOS
...
# --- ESCRITURA FINAL MASIVA (SOLO 1 PETICIÓN A GOOGLE) ---
if filas_para_google:
    res_sheet.append_rows(filas_para_google) # <--- AQUÍ SE SUBE TODO DE GOLPE
