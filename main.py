...
def draw_justified_efe_lc(draw, text, font_reg, font_bold, y_start, x_start, x_end, fill, line_spacing=1):
    """Dibuja legales justificados con prefijo en negrita."""
...
def draw_efe_preciador(draw, x_center, y_center, text_s, text_price, f_ps, f_pv, scale=1.0):
    """Preciador EFE adaptativo con dígitos compactos."""
...
def generar_diseno(data_input, color_version="AMARILLO"):
    ...
    # --- DEFINICIÓN GLOBAL DE COLORES (Solución al NameError) ---
    if tienda == "EFE":
        txt_c = (255, 255, 255)
    else: # LC
    ...
    if formato == "FLYER":
        for i, (idx, p) in enumerate(data_input.iterrows()):
            ...
            if tienda == "EFE":
                ...
                draw.text((xp+115, yp+320), p['Marca'], font=f_m, fill=(0,0,0), anchor="mm")
        ...
        if formato == "PPL":
            if tienda == "EFE":
                if "EFERTON" in tipo:
                    ...
                    draw_justified_efe_lc(draw, str(row['Legales']), f_l, f_l_b, 970, 50, 950, (255,255,255))
            else: # LC PPL
                ...
                ny = 835; [draw.text((275, ny), l, font=f_p, fill=txt_c, anchor="mm") for l in textwrap.wrap(row['Nombre del producto'], width=22)]; ny+=34
                ...
                draw.text((px + 80, 780), str(row['Precio desc']), font=f_pv, fill=txt_c, anchor="mm")
                draw_justified_efe_lc(draw, str(row['Legales']), f_l, f_l_b, 945, 50, 950, txt_c)
            ...
            if tienda == "EFE":
                ...
                draw.text((px + 85, ay-50), str(row['Precio desc']), font=f_pv, fill=txt_c, anchor="mm")
