...
# --- LÓGICA DE POSICIONAMIENTO FLYER (Múltiples productos) ---
    ...
    if formato == "FLYER":
        ...
        draw.rounded_rectangle([x_fecha, 235, x_fecha+wf+35, 285], radius=10, outline=color_fecha, width=3)
        draw.text((x_fecha+(wf+35)//2, 260), f_txt, font=f_f, fill=color_fecha, anchor="mm")
        ...
        for i, (idx, p) in enumerate(data_input.iterrows()):
            ...
            # --- FORMATEO DE PRECIO PARA FLYER ---
            try:
                p_val = "{:,}".format(int(float(p['Precio desc'])))
            except:
            ...
            draw.rounded_rectangle([xp, yp, xp+box_w, yp+box_h], radius=15, fill=(255,255,255), outline=(255, 255, 255), width=2)
        ...
        draw.text((64, y_legales_fijo), tit_legal, font=f_l_bold, fill=(0, 0, 0))
        draw_justified_text(draw, cuerpo_legal, f_l, y_legales_fijo, 64, 1016, (0, 0, 0), line_spacing=2, prefix_width=ancho_negrita)
    ...
    # --- LÓGICA DE POSICIONAMIENTO FORMATOS INDIVIDUALES ---
        ...
        if formato == "DISPLAY":
            ...
            draw_justified_text(
                ...
                y_start=485, x_start=40, x_end=960, 
            ...
            pi.thumbnail((847, 650), Image.Resampling.LANCZOS)
