...
def get_sheets_data():
    ...
    if len(valores_actuales) > 1:
        for row in valores_actuales[1:]:
            if len(row) >= 2:
                registros_viejos.add(row[1].upper()) # El ID único está en la columna 2
...
def generar_diseno(data_input, color_version="AMARILLO"):
    ...
    # Colores dinámicos para bordes según el fondo
    border_color = (254, 215, 0) if color_version == "AMARILLO" else (10, 6, 60) # #FED700 vs #0A063C
    ...
    # --- FUENTES ACTUALIZADAS (Centralizadas en TIPOGRAFIA/LC/) ---
    try:
        f_marca = ImageFont.truetype(f"{path_fonts}/HurmeGeometricSans1 Bold.otf", 42)
        f_prod = ImageFont.truetype(f"{path_fonts}/HurmeGeometricSans1 Bold.otf", 22)
    ...
    if formato == "FLYER":
        try:
            ...
            # 2. Cuadrícula adaptable (Bajada para dar aire arriba)
            num_prod = len(data_input)
            box_h = 330 if num_prod > 4 else 460
            start_y = 340 # Más espacio arriba
            ...
            for i, (idx, p_row) in enumerate(data_input.iterrows()):
                ...
                p_img.thumbnail((box_h-130, box_h-160))
                img.paste(p_img, (int(xp + (460-p_img.width)//2), int(yp + 20)), p_img)

                # Textos
                cl, cr = xp + 115, xp + 345
                draw.text((cl, yp+box_h-95), p_row['Marca'], font=f_marca, fill=(0,0,0), anchor="mm")
                ...
                px = cr - (w_s + w_v + 5)//2
                draw.text((px, yp+box_h-70), "S/", font=f_precio_sim, fill=(0,0,0), anchor="lm")
                draw.text((px + w_s + 5, yp+box_h-70), p_str, font=f_precio_val, fill=(0,0,0), anchor="lm")
                draw.text((cr, yp+box_h-20), str(p_row['SKU']), font=f_sku, fill=(100,100,100), anchor="mm")

            # 3. Legales con márgenes
            wrapped_leg = textwrap.fill("CONDICIONES GENERALES: " + str(row['Legales']), width=110)
            draw.text((65, 1840), wrapped_leg, font=f_leg_reg, fill=txt_color)

        except: return None

    else:
        # DISPLAY, STORY, PPL (Corregidos para que siempre se generen)
        try:
            ...
            if formato == "DISPLAY":
                ...
                for l in textwrap.wrap(row['Nombre del producto'], width=20):
                    draw.text((cx, ny), l, font=f_prod, fill=txt_color, anchor="mt"); ny += 28
                # Precio
                ...
                draw.text((cx, ny+110), str(row['SKU']), font=f_sku, fill=txt_color, anchor="mt")
                draw.text((35, 465), textwrap.fill("CONDICIONES GENERALES: " + str(row['Legales']), width=95), font=f_leg_reg, fill=txt_color)
                ...
                ny = 1475
                for l in textwrap.wrap(row['Nombre del producto'], width=18):
                    draw.text((270, ny), l, font=f_prod, fill=txt_color, anchor="mt"); ny += 35
                p_str = str(row['Precio desc'])
                tw = draw.textlength("S/ ", font=f_precio_sim) + draw.textlength(p_str, font=f_precio_val)
                px = 810 - tw//2
                draw.text((px, 1470), "S/ ", font=f_precio_sim, fill=txt_color, anchor="mm")
                draw.text((px + 50, 1470), p_str, font=f_precio_val, fill=txt_color, anchor="mm")
                draw.text((810, 1550), str(row['SKU']), font=f_sku, fill=txt_color, anchor="mm")
                draw.text((65, 1845), textwrap.fill("CONDICIONES GENERALES: " + str(row['Legales']), width=105), font=f_leg_reg, fill=txt_color)
                ...
                ny = 810
                for l in textwrap.wrap(row['Nombre del producto'], width=20):
                    draw.text((275, ny), l, font=f_prod, fill=txt_color, anchor="mt"); ny += 32
                p_str = str(row['Precio desc'])
                tw = draw.textlength("S/ ", font=f_precio_sim) + draw.textlength(p_str, font=f_precio_val)
                px = 735 - tw//2
                draw.text((px, 810), "S/ ", font=f_precio_sim, fill=txt_color, anchor="mm")
                draw.text((px + 50, 810), p_str, font=f_precio_val, fill=txt_color, anchor="mm")
                draw.text((735, 885), str(row['SKU']), font=f_sku, fill=txt_color, anchor="mm")
                draw.text((45, 940), textwrap.fill("CONDICIONES GENERALES: " + str(row['Legales']), width=100), font=f_leg_reg, fill=txt_color)
        except: return None
...
# 1. Procesar Individuales (Story, PPL, Display)
