...
def generar_diseno(data_input, color_version="AMARILLO"):
    ...
    # Búsqueda de fondo considerando la palabra "FONDO" en el nombre (según imagen e0f5c1)
    base_name = f"LC - {tipo} - {formato}"
    if formato == "FLYER": base_name = f"LC - {tipo} - FLYER"
    
    full_path_fondo = None
    # Probamos las dos variantes de nombres que tienes en tus carpetas
    for variant in [f"{base_name} FONDO {color_version}", f"{base_name} {color_version}", f"{base_name}"]:
    ...
    if not full_path_fondo:
        print(f"Fondo no hallado: {base_name} {color_version} en {path_fondos}")
        return None
    ...
    try:
        f_marca = ImageFont.truetype(f"{path_fonts}/HurmeGeometricSans1 Bold.otf", 38)
        f_prod = ImageFont.truetype(f"{path_fonts}/HurmeGeometricSans1 Bold.otf", 20)
        f_pre_val = ImageFont.truetype(f"{path_fonts}/HurmeGeometricSans1 Bold.otf", 68)
        f_pre_sim = ImageFont.truetype(f"{path_fonts}/HurmeGeometricSans1 Bold.otf", 30)
        f_sku = ImageFont.truetype(f"{path_fonts}/HurmeGeometricSans1.otf", 13)
        f_fecha = ImageFont.truetype(f"{path_fonts}/HurmeGeometricSans1 Bold.otf", 24)
    ...
    if formato == "FLYER":
        try:
            # 1. Fecha a la Izquierda (Alineada a OFERTA IMPERDIBLE)
            fecha_txt = str(row['Fecha_disponibilidad_flyer']).upper()
            w_f = draw.textlength(fecha_txt, font=f_fecha)
            # Rectángulo en la parte izquierda (x=75)
            draw.rounded_rectangle([75, 235, 75 + w_f + 35, 285], radius=10, outline=border_color, width=3)
            draw.text((75 + (w_f + 35)//2, 260), fecha_txt, font=f_fecha, fill=border_color, anchor="mm")

            # 2. Cuadrícula Adaptable
            num_p = len(data_input)
            # Si hay 6 o menos, damos más altura. Si hay 8, compactamos.
            box_h = 330 if num_p > 6 else 450
            start_y = 345 # Bajamos para dar aire arriba
            
            for i, (idx, p_row) in enumerate(data_input.iterrows()):
                if i >= 8: break
                col, fila = i % 2, i // 2
                xp, yp = 65 + (col * 495), start_y + (fila * (box_h + 12))
                
                # Cuadros con bordes dinámicos
                ...
                img.paste(p_img, (int(xp + (455-p_img.width)//2), int(yp + 20)), p_img)

                # Info interna
                cl, cr = xp + 115, xp + 345
                draw.text((cl, yp+box_h-100), p_row['Marca'], font=f_marca, fill=(0,0,0), anchor="mm")
                
                ny = yp + box_h - 55 # Más distancia con la marca
                wrap_n = textwrap.wrap(p_row['Nombre del producto'], width=18)
                for line in wrap_n[:2]:
                    draw.text((cl, ny), line, font=f_prod, fill=(0,0,0), anchor="mm"); ny += 22
        ...
        try:
            ...
            if formato == "DISPLAY":
                ...
                draw.text((40, 470), textwrap.fill("CONDICIONES GENERALES: " + str(row['Legales']), width=110), font=f_leg_reg, fill=txt_color)
                ...
                draw.text((65, 1850), textwrap.fill("CONDICIONES GENERALES: " + str(row['Legales']), width=115), font=f_leg_reg, fill=txt_color)
                ...
                draw.text((45, 945), textwrap.fill("CONDICIONES GENERALES: " + str(row['Legales']), width=115), font=f_leg_reg, fill=txt_color)
