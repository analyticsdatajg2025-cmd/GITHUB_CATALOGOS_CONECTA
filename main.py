...
# --- AUTO-AJUSTE DE NOMBRES LARGOS ---
def envolver_por_ancho(draw, texto, fuente, ancho_max):
    """Envuelve el texto por ANCHO REAL en px (no por número de caracteres)."""
    palabras = str(texto).split()
    lineas, actual = [], ""
    for w in palabras:
        prueba = (actual + " " + w).strip()
        if not actual or draw.textlength(prueba, font=fuente) <= ancho_max:
            actual = prueba
        else:
...
def ajustar_nombre(draw, texto, ruta_fuente, size_max, size_min, ancho_max, max_lineas):
    """Devuelve (lineas, fuente, alto_linea). Reduce la fuente hasta que entre en
    max_lineas. NUNCA corta el texto: si llega al size_min muestra todo igual."""
    size = size_max
    fuente = ImageFont.truetype(ruta_fuente, size)
    lineas = envolver_por_ancho(draw, texto, fuente, ancho_max)
    while len(lineas) > max_lineas and size > size_min:
        size -= 1
        fuente = ImageFont.truetype(ruta_fuente, size)
        lineas = envolver_por_ancho(draw, texto, fuente, ancho_max)
...
def generar_diseno(data_input, color_version="AMARILLO"):
    ...
    if formato == "FLYER":
        ...
        if "EFERTON" in tipo:
            ...
            draw.text((x_fecha_der, y_fecha_centro), f_txt, font=f_f_semibold, fill=(255,255,255), anchor="lm")
        else:
        ...
        # AJUSTE: cuando el flyer tiene exactamente 4 productos (2 filas reales dentro de un
        # bloque calculado para 3 filas) queda mucho vacío abajo. Bajamos el bloque ~180px
        # extra para que se vea más centrado respecto al espacio realmente ocupado.
        # Los legales (1835) no se tocan; con 2 filas de box_h=430 el bloque baja hasta
        # current_y_top + 872, dejando margen de sobra antes de los legales.
        if num_prod == 4:
            current_y_top += 230
        ...
        for i, (idx, p) in enumerate(data_input.iterrows()):
            ...
            lineas_n, f_nom, alto_n = ajustar_nombre(draw, p['Nombre del producto'], ruta_nombre, size_max=20, size_min=13, ancho_max=215, max_lineas=3)
            for line in lineas_n:
            ...
            if "EFERTON" in tipo:
                # AJUSTE: Sin preciador naranja. Precio y S/ en color naranja (igual que IRRESISTIBLE)
                y_p_efe = yp + box_h - 88
        ...
        if formato == "PPL":
            if "EFERTON" in tipo:
                # AJUSTE: Replica del diseño de IRRESISTIBLE pero SOLO con preciador (sin precio en texto)
                # AJUSTE EFERTON: imagen bajada 40px (287 -> 327). El resto queda igual.
                pi.thumbnail((682, 682)); img.paste(pi, (310, 327), pi)
            ...
            if "EFERTON" in tipo:
                # AJUSTE EFERTON: imagen 10px más pequeña por lado (510 -> 500),
                # bajada 10px (10 -> 20) y movida 10px a la izquierda (430 -> 420).
                # El borde inferior de la imagen queda igual que antes (20+500 = 10+510 = 520),
                # así que no se acerca más a los legales (y=485, que corren en la columna izquierda).
                pi.thumbnail((500, 500)); img.paste(pi, (420, 20), pi); cx = 260
