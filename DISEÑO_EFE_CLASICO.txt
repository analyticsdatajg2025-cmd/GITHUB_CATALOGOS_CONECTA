    ...
    if formato == "FLYER":
        ...
        for i, (idx, p) in enumerate(data_input.iterrows()):
            ...
            if p_hay_precio:
                ...
                cx_uni = xp + 240
                draw.text((cx_uni, yp + box_h - 115), p['Marca'], font=f_m_flyer, fill=(0,0,0), anchor="mm")
                y_n = yp + box_h - 85
                for line in textwrap.wrap(str(p['Nombre del producto']), width=26)[:3]:
                    draw.text((cx_uni, y_n), line, font=f_p, fill=(0,0,0), anchor="mm"); y_n += 22
                draw.text((cx_uni, y_n + FLYER_GAP_SKU), str(p['SKU']), font=f_s_ind, fill=(0,0,0), anchor="mm")
        ...
        if formato == "PPL":
            if "EFERTON" in tipo:
                ...
                if hay_precio:
                    # 3 columnas: marca | nombre/SKU | preciador
                    ...
                    # 2 columnas: marca | nombre/SKU (el nombre se recentra en el espacio libre)
